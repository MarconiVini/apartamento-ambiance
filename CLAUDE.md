...
- **Tailwind via Play CDN** (`https://cdn.tailwindcss.com?plugins=forms,container-queries`) — plugins `forms` e `container-queries` ativos. A config do Tailwind (cores/tokens mapeados) vive **dentro de `resources/app.js`**, não no HTML.
- Ícones: **Bootstrap Icons 1.11.3** + Material Symbols + Google Fonts (Hanken Grotesk, Inter).
- GA4 `G-R40NFZHCLM` (gtag no `<head>`; eventos de visualização/permanência/cliques por seção no `resources/app.js`).
...
`resources/app.js` é carregado **de forma síncrona no `<head>`** do `ambiance-1/index.html`, logo após o Play CDN, para que os scripts Anti-FOUC (tema, WhatsApp) rodem antes da primeira pintura e a config do Tailwind seja aplicada a tempo. Não transforme em `defer`/`async` nem mova para o fim do body.

## Tema (light/dark)

- Tokens de cor são **canais rgb** (`--on-surface: 26 28 28`) definidos em `resources/app.css`, acessados via `rgb(var(--token) / <alpha>)` tanto pelo Tailwind quanto pelo CSS customizado.
- Troca de tema = adicionar/remover classe `.dark` no `<html>` (prioridade: `localStorage` > `prefers-color-scheme`).
- **Gotcha:** o `resources/app.css` é um arquivo comum, **não** processado pelo Tailwind — `theme()` ali é ignorado. Use os hex/rgb do `ambiance-1/DESIGN.md`.
...
- **CTAs de WhatsApp ficam ocultos por padrão** e o desbloqueio é persistente em `localStorage` (key `wa-enabled`, escopo por origem — um único flag vale para o hub e os sites). `?magic=awesome` liga para sempre; `?magic=<qualquer outro valor>` desliga; sem parâmetro, usa o valor salvo (nada salvo = oculto). O parâmetro é removido da URL sem reload após uso, preservando os demais. A lógica vive no IIFE do `resources/app.js` e num pequeno script inline no `<head>` do hub raiz (que não carrega o `app.js`). Não "corrija" CTAs invisíveis — é intencional.
- **Reveal é estático por decisão do dono (16/08/2026):** todos os `.reveal-up` já nascem com `active` — o conteúdo fica sempre visível, mesmo se o JS falhar. O estado oculto e a transition longa vivem em `.reveal-up:not(.active)` no `resources/app.css`. Ao criar elementos novos com `.reveal-up`, inclua a classe `active`.
